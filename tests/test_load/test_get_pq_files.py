...
import json
import boto3
from src.load.First_util_for_3rd_lambda import get_pq_files
...
def S3_setup():
    with mock_aws():
        ...
        mock_list_of_keys = [
            "2025-06-06 12:34:56/date.parquet",
            "2025-06-06 12:34:56/sales_xxx.parquet",
        ]

        # for the date table:
        data_1 = {"name": ["Abdul", "Neill", "Mukund"], "team": [1, 1, 1]}
        # for the staff table:
        data_2 = {"name": ["Abdul", "Neill", "Mukund"], "team": [1, 1, 1]}
        ...
        S3_client.put_object(
            Bucket="11-processed-bucket",
            Key="2025-06-06 12:34:56/date.parquet",
            Body=buffer,
        )
        ...
        S3_client.put_object(
            Bucket="11-processed-bucket",
            Key="2025-06-06 12:34:56/staff.parquet",
            Body=buffer,
        )
...
def test_get_pq_files_does_something(S3_setup):
    ...
    mock_list_of_keys = [
        "2025-06-06 12:34:56/date.parquet",
        "2025-06-06 12:34:56/staff.parquet",
    ]

    # make date table:
    expected_date_table = {"name": ["Abdul", "Neill", "Mukund"], "team": [1, 1, 1]}

    # make staff table:
    expected_staff_table = {"name": ["Abdul", "Neill", "Mukund"], "team": [1, 1, 1]}
